/**
 * @Author: vickiWu
 * @Date: 2022-03-23 18:16:21
 * @LastEditTime: 2022-03-23 18:19:26
 * @LastEditors: vickiWu
 * @Description: md语法解析 loader入口文件
 * @FilePath: \wwy-map\build\md-loader\index.js
 */
...
module.exports = function(source) {
  ...
  // 注释Tag 开始结束的名称和长度
  const startTag = '<!--d5c-demo:';
  const startTagLen = startTag.length;
  const endTag = ':d5c-demo-->';
  ...
  while (commentStart !== -1 && commentEnd !== -1) {
    ...
    const demoComponentName = `d5c-demo${id}`; // 内联组件名称

    output.push(`<template slot="source"><${demoComponentName} /></template>`); // 使用slot插槽 运行组件
    componentsString += `${JSON.stringify(demoComponentName)}: ${demoComponentContent},`; // 页面组件注册   组件名称:组件内容
